...
running, by copying the base config directly into startup-config then
reloading.  Works for EVPN, SDA, manual config, or any other state.

Flow (single pass):
  1. SSH connect           (netadmin / C1sco12345)
  2. Enable SCP server     (temporarily — not in base config, removed on reload)
  3. SCP base config       → flash:oc_reset_base.txt
  4. write erase           (clear NVRAM startup-config)
  5. copy flash:oc_reset_base.txt startup-config  (populate NVRAM with base config)
  6. Flash cleanup         (delete EVPN/CatC residual flash files that could
                            override startup-config on boot)
  7. crypto key generate   (SSH ready immediately after reload)
  8. reload                (switch boots cleanly from base config in NVRAM)
  9. Wait 300 s
 10. SSH verify + final write memory
...
def _reset_to_base(host, local_config_path, log_fn):
    """
    Single-pass SSH reset.

    Transfers the base config to flash then copies it into startup-config
    (NVRAM) before reloading.  The switch boots cleanly from the base config
    regardless of what was previously running on it.
    """
    ...
    # Enable SCP server so paramiko can push the file.  It is not in the base
    # config so it will be absent after the reload — no explicit cleanup needed.
    ...
    # ── Step 2: Clear NVRAM then copy base config into startup-config ─────
    # write erase empties NVRAM.  The subsequent copy populates it with the
    # base config — identical to what a clean new switch would have.
    # Running config is NOT touched; the switch continues to operate normally
    # until the reload in step 6.
    ...
    log_fn(f"  Copying base config to startup-config...")
    out = conn.send_command(
        f"copy flash:{_REMOTE_CFG} startup-config",
        expect_string=r"\[startup-config\]|#|\[confirm\]",
        read_timeout=30,
    )
    # IOS XE prompts "Destination filename [startup-config]?" — accept default
    if "startup-config" in out and "#" not in out[-5:]:
        out2 = conn.send_command("\n", expect_string=r"#", read_timeout=30)
        out += out2
    log_fn(f"  copy to startup-config: {out.strip()[-80:]}")

    # ── Step 3: Delete EVPN/CatC flash files ─────────────────────────────
    # These take priority over NVRAM on C9300 and would reload the old config
    # if left in place.
